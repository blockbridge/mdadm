...
{
	pr_err("RAID%d layout 0x%x with %d disks is unsupported for DDF\n",
			  ...
			  int mdvote, int runstop)
{
	...
	/* Put this at bottom of 32MiB reserved.. */
...
static void _set_config_size(struct phys_disk_entry *pde, const struct dl *dl)
{
	...
	dprintf("%d:%d config_size 0x%llx, DDF structure is 0x%llx blocks\n",
		dl->major, dl->minor,
			    ...
			    unsigned long long data_offset)
{
	...
	if (size <= 32*1024*1024) {
		pr_err("device size must be at least 32MiB\n");
		return 1;
	}
	size >>= 9;
	...
	*tptr = random32();
	 ...
	 * amount of workspace. Otherwise, use 32MiB.
	...
	for (vc = ddf->conflist ; vc ; vc=vc->next) {
		...
		for (pd = 0; pd < be16_to_cpu(ddf->phys->max_pdes); pd++) {
			...
			if ((stt & (DDF_Online|DDF_Failed|DDF_Missing|DDF_Rebuilding)) != DDF_Online) {
				dprintf("-> skipping pdnum:%d refnum:%08x state:%s:%u not online\n",
					pd,
					be32_to_cpu(refnum),
			...
			i = get_pd_index_from_refnum(
				...
				ddf->mppe, &bvd, &iphys);
			if (i == DDF_NOTFOUND) {
				dprintf("-> skipping pdnum:%d refnum:%08x state:%s:%u phys disk index not found\n",
					pd,
					be32_to_cpu(refnum),
					ddf_device_state_str(stt),
					stt);
			...
			if (d == NULL) {
				...
				dprintf("-> skipping pdnum:%d refnum:%08x state:%s:%u disk refnum not found in dlist\n",
					pd,
					be32_to_cpu(refnum),
					ddf_device_state_str(stt),
					stt);
			...
			dprintf("-> found pdnum:%d refnum:%08x raiddisk:%d (%d:%d)\n",
				pd,
				be32_to_cpu(refnum),
				i,
				d->major,
...
static int ddf_open_new(struct supertype *c, struct active_array *a, char *inst)
{
	...
	for (dev = a->info.devs; dev; dev = dev->next) {
		...
		if ((be16_to_cpu(ddf->phys->entries[dl->pdnum].state) &
			...
			pr_err("new subarray %d contains broken device %d:%d (0x%02x)\n",
			       n, dl->major, dl->minor,
...
static void ddf_set_disk(struct active_array *a, int n, int state)
{
	...
	dprintf("ddf: set_disk %d (%08x) to 0x%x->0x%02x%s\n", n,
